...
def span_gauge(post_w, span_pitches=16, gap_pitches=8, rows=2):
    """A bar with posts only at 0, gap and span pitches, in two rows.

    Seats only if the printer's cumulative scaling error across the span stays
    inside the post-to-hole play. Two rows so it cannot pivot and fake a pass.
    """
...
if __name__ == "__main__":
    ...
    for w in (2.0, 2.2, 2.4):
        tris, size = span_gauge(w)
        f = OUT / f"span_gauge_post{w:.1f}mm.stl"
        write_stl(f, tris, f"span gauge 16 pitches post {w:.2f}mm".encode())
